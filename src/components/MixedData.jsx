...
const MixedData = () => {
  // Kartlarning xolatlarini saqlash uchun state
  const [cardStates, setCardStates] = useState(Array(mixedData.length).fill(false));

  // Kartni bosganda xolatni o'zgartirish funksiyasi
          ...
          return (
            <li key={id} className={`bg-white p-5 rounded-lg hover:shadow-lg relative ${cardStates[id] ? 'liked' : 'not-liked'}`}>
              <img
                width={200}
                height={200}
                className='mb-4 hover:transition-transform hover:origin-center'
                src={e.img}
                alt={e.title}
              />
              <button onClick={() => toggleCard(id)} className={cardStates[id] ? 'liked' : 'not-liked'}>
                <svg
                  width="30"
                  height="30"
                  version="1.1"
